...
IUSE="clippy cpu_flags_x86_sse2 debug doc libressl miri nightly parallel-compiler rls rustfmt system-bootstrap system-llvm wasm ${ALL_LLVM_TARGETS[*]} std_targets_aarch64"
...
LLVM_DEPEND="
	...
	<sys-devel/llvm-11:=
	wasm? ( sys-devel/lld )
	std_targets_aarch64? ( sys-devel/llvm[llvm_targets_AArch64] )
"
LLVM_MAX_SLOT=10
...
REQUIRED_USE="|| ( ${ALL_LLVM_TARGETS[*]} )
	...
	std_targets_aarch64? ( llvm_targets_AArch64 )
"
...
src_configure() {
	...
	# patch for extern std targets
	use std_targets_aarch64 && rust_targets="${rust_targets},\"aarch64-unknown-linux-gnu\""
	...
	# patch for extern std targets
	use std_targets_aarch64 && cat <<- EOF >> "${S}"/config.toml
